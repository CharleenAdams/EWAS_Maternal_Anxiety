###################################################################################################################
###################################################################################################################
#####                                Maternal Anxiety EWAS Analysis                                           #####
#####                         Compiled 03/09/2018 by Analyst Charleen Adams                                   #####
#####                       Adapted from Gemma Sharp's Code for EWAS with MEFFIL                              #####
###################################################################################################################
###################################################################################################################

#Bash script command that runs this code:
#R CMD BATCH --no-save --no-restore '--args anx.contwins andrews-and-bakulski Cells Phenofile B cord mat.age.wins,mat.ses,sex,bw.wins,gest.age,mat.smoke,psych.meds /panfs/panasas01/sscm/ca16591/EWAS/' /panfs/panasas01/sscm/ca16591/Common_files/meffil_EWAS_script.r /panfs/panasas01/sscm/ca16591/EWAS/pace_mat_anx/ewas_results/mat_anx_score.out

# pull in args: set manually for running interactively in R as a test of the code
#args	<- 	commandArgs(trailingOnly = TRUE)
#Trait 	<- 	toString("anx.contwins") #Phenotypic exposure or outcome of interest
#CellData <-toString("andrews-and-bakulski") #Which cell counts should we use? (houseman, gse68456, gervinandlyle, andrews-and-bakulski, houseman_eos)
#CellAdj   <-      toString("Cells") #Cell adjusted? (noCells, Cells)
#Phenofile       <-      toString("andrews-and-bakulski") #Path to file containing all phenotype information (must be a dta stata version 12)
#BorM    <-      toString("B") #Betas or M-values (B or M)
#TP <-  toString("cord") #Time point (cord or F7 or 15up or antenatal or FOM)
#Covariates <-toString("mat.age.wins,mat.ses,sex,bw.wins,gest.age,mat.smoke,psych.meds") #list of covariates (eg: m_age,mum_uni,matsm,parity i.e. commas but no spaces or quotation marks)
#WD<- toString("/panfs/panasas01/sscm/ca16591/EWAS/") #Working directory (eg /panfs/panasas01/sscm/gs8094/EWAS/example_project)
...
#devtools::install_github("perishky/meffil", repos='https://www.stats.bris.ac.uk/R/')
...
load("/panfs/panasas01/dedicated-mrcieu/studies/latest/alspac/epigenetic/methylation/450k/aries/released/2016-05-03/data/samplesheet/data.Robj", verbose=TRUE)
samplesheet<-subset(samplesheet, time_point==TP)
if(TP !="antenatal" & "cord" !="FOM"){
...
load("/panfs/panasas01/dedicated-mrcieu/studies/latest/alspac/epigenetic/methylation/450k/aries/released/2016-05-03/data/detection_p_values/data.Robj", verbose=TRUE)
...
#Pheno<-read.csv(paste0(Phenofile,".csv"),stringsAsFactors=FALSE)# Doesn't work...

Pheno<-read.csv(paste0("/panfs/panasas01/sscm/ca16591/EWAS/pace_mat_anx/meth_merged_winsored.pheno2.csv"),stringsAsFactors=FALSE)
...
paste("There are now ",nrow(Pheno), " people in the phenofile; some had dduplicate Sample_Name")

#Remove duplicate Sample_Names (there were 12)
duplicates=duplicated(Pheno[,1], incomparables=FALSE)
Pheno$duplicates=duplicates
Pheno$dups=as.numeric(Pheno$duplicates)
Pheno_clean<-Pheno[!(Pheno$dups=="1"),]
dim(Pheno_clean)
colnames(Pheno_clean)
Pheno=Pheno_clean[c(-27, -29)]
paste("There are now ",nrow(Pheno), " people in the phenofile; duplicate Sample_Name removed")

#Save the merged methylation-phenotype file to run the summary descriptives for PACE (done initially--not needed for final)
write.csv(Pheno, "/panfs/panasas01/sscm/ca16591/EWAS/pace_mat_anx/MergedMethylationALN_Phenofile3.csv")

#Prepare phenotype data
Covs<-strsplit(Covariates,split=",")[[1]]
Pheno<-na.omit(Pheno[,c("Sample_Name",Trait,Covs)])

#Pheno2<-na.omit(Pheno[,c("Sample_Name","anx.contwins","mat.smoke","gest.age","bw.wins","psych.meds",
#						"mat.age.wins","sex","mat.ses", 
...
#Match meth to Pheno
meth2<-meth[,na.omit(match(Pheno$Sample_Name,colnames(meth)))]
paste("There are now ",ncol(meth2), " people in the methylation file")
#meth<-meth[,na.omit(match(Pheno$Sample_Name,colnames(meth)))]
Pheno2<-Pheno[match(colnames(meth2),Pheno$Sample_Name),]
paste("There are now ",nrow(Pheno2), " people in the phenofile")
Pheno=Pheno2
meth=meth2
ifelse(all(Pheno$Sample_Name==colnames(meth2)), "meth and phenotype data successfully matched :) ","Data not matched :(")
...
ewas.parameters<-meffil.ewas.parameters(sig.threshold=1e-5, max.plots=5, model="sva")
...
meffil.ewas.report(ewas.summary, output.file=savefile,author="Charleen Adams & Gemma Sharp", study="alspac")
...
Naeem<-read.csv("/panfs/panasas01/sscm/ca16591/Common_files/naeem_list.csv")
